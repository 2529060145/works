...
<AppCard class="import-card"><div class="drop-zone"><el-icon :size="34"><Upload/></el-icon><h2>导入 .xlsx 或 .xls 工作簿</h2><p>自动查找真实表头并跳过标题、统计行；同一企业的连续岗位可省略企业名称。岗位名称是唯一核心字段，其余信息均可留空。</p><el-button :disabled="!isTauriRuntime()" @click="choose">浏览文件</el-button></div><div class="columns"><strong>可识别字段</strong><span v-for="item in ['企业名称','企业性质','工作地点','岗位名称','招聘批次','薪资','学历要求','专业要求','岗位要求','招聘人数','发布日期','截止日期','网址链接','投递阶段','企业投递日期','投递结果','备注']" :key="item">{{ item }}</span></div></AppCard>
