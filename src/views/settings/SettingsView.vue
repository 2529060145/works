...
import { getPortableDataPaths, isTauriRuntime } from '../../services/databaseService'
...
async function load(){if(!isTauriRuntime())return;const paths=await getPortableDataPaths();databaseDirectory.value=paths.dataDirectory;attachmentDirectory.value=paths.attachmentDirectory;const values=await getSettings();form.userName=values.user_name||'用户';form.theme=values.theme==='dark'?'dark':'light';store.userName=form.userName;store.theme=form.theme;store.applyTheme()}
async function openLocal(path:string){await invoke('open_portable_path',{path})}
...
<div class="settings-grid"><AppCard class="settings-card"><h2>个人偏好</h2><el-form label-position="top"><el-form-item label="称呼"><el-input v-model="form.userName" maxlength="20"/></el-form-item><el-form-item label="界面主题"><el-segmented v-model="form.theme" :options="[{label:'浅色',value:'light'},{label:'深色',value:'dark'}]"/></el-form-item></el-form></AppCard><AppCard class="settings-card"><h2>本地数据</h2><p>数据库、附件和应用设置保存在客户端程序旁的 data 文件夹，不会上传云端或写入系统 AppData。</p><label class="path-label">数据库目录</label><div class="path-box"><code>{{ databaseDirectory }}</code><el-button :icon="FolderOpened" :disabled="!isTauriRuntime()" @click="openLocal(databaseDirectory)">打开</el-button></div><label class="path-label">附件目录</label><div class="path-box"><code>{{ attachmentDirectory }}</code><el-button :icon="FolderOpened" :disabled="!isTauriRuntime()" @click="openLocal(attachmentDirectory)">打开</el-button></div><dl><div><dt>数据库</dt><dd>data\job_manager.db</dd></div><div><dt>存储方式</dt><dd>便携式 SQLite，跟随程序目录</dd></div></dl></AppCard></div></div></template>
