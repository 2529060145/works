...
<div class="metric-grid"><AppCard><span>收录岗位</span><strong>{{ data.totalJobs }}</strong></AppCard><AppCard><span>有效机会</span><strong>{{ data.effectiveOpportunities }}</strong></AppCard><AppCard><span>当前面试</span><strong>{{ data.workflow.interviews }}</strong></AppCard><AppCard><span>Offer 转化率</span><strong>{{ offerRate }}%</strong></AppCard></div>
