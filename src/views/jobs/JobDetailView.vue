...
function timelineTone(item:WorkflowNode){if(item.status==='CANCELLED'||item.result==='CANCELLED')return'cancelled';if(item.result==='FAILED')return'failed';if(item.status==='COMPLETED'&&item.result==='PASSED')return'passed';if(item.status==='SCHEDULED'||item.status==='PENDING_SCHEDULE')return'scheduled';return'pending'}
function timelineColor(item:WorkflowNode){return{cancelled:'var(--text-tertiary)',failed:'var(--danger)',passed:'var(--success)',scheduled:'var(--warning)',pending:'var(--primary)'}[timelineTone(item)]}
...
<template><div v-loading="loading" class="page-stack">
  ...
  <AppCard class="timeline-card"><div class="section-head"><h2>招聘流程时间轴</h2><el-button v-if="job.stage==='PROCESS'" :icon="Right" link type="primary" @click="router.push('/workflow')">管理流程</el-button></div><el-timeline v-if="events.length"><el-timeline-item v-for="item in events" :key="item.key" :class="`workflow-${timelineTone(item)}`" :color="timelineColor(item)" :timestamp="displayDateTime(item.scheduledAt,item.timeTbd)" placement="top"><strong>{{ item.label }}</strong><p>{{ workflowStatusLabels[item.status] }} · {{ workflowResultLabels[item.result] }}<template v-if="item.location"> · {{ item.location }}</template><el-button v-if="item.meetingUrl&&isWebLink(item.meetingUrl)" :icon="Link" link type="primary" title="打开流程链接" @click="openWebLink(item.meetingUrl)"/></p></el-timeline-item></el-timeline><EmptyState v-else title="暂无流程记录" description="进入面试/笔试阶段后，请在招聘流程页面安排第一个节点。"/></AppCard></div></template>
