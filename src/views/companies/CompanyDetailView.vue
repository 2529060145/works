...
import { getCompanyApplicationLimit, type CompanyApplicationLimit } from '../../services/applicationEligibilityService'

const route=useRoute(),router=useRouter(),company=ref<Company|null>(null),limit=ref<CompanyApplicationLimit|null>(null),jobs=ref<Job[]>([]),loading=ref(false)
const dialog=ref<InstanceType<typeof JobDialog>>()
async function load(){loading.value=true;try{const id=Number(route.params.id);company.value=await getCompany(id);limit.value=await getCompanyApplicationLimit(id);jobs.value=(await listJobs({companyId:id,pageSize:100})).items}catch(e){ElMessage.error(e instanceof Error?e.message:'读取企业详情失败')}finally{loading.value=false}}
...
<template><div v-loading="loading" class="page-stack">
  <PageHeader :title="company?.companyName ?? '企业详情'" :subtitle="company?.companyType || '企业资料与岗位汇总'">
    <el-button :icon="ArrowLeft" @click="router.push('/companies')">返回</el-button><el-button :icon="Plus" type="primary" @click="company && dialog?.open(undefined, company)">新增岗位</el-button>
  </PageHeader>
  <div v-if="company" class="detail-grid"><AppCard class="profile-card"><h2>企业档案</h2><div v-if="limit" class="limit-panel" :class="{reached:limit.limitReached}"><strong>{{ limit.applicationLimitType==='LIMITED'?`最多可投 ${limit.maxApplications} 个`:limit.applicationLimitType==='UNLIMITED'?'投递数量不限制':'投递限制未知' }}</strong><span>已投 {{ limit.appliedCount }} 个<template v-if="limit.remainingSlots!=null"> · 剩余 {{ limit.remainingSlots }} 个</template></span><StatusTag v-if="limit.limitReached" type="danger">已达投递上限</StatusTag></div><dl><div><dt>总部</dt><dd>{{ company.headquarters||'未填写' }}</dd></div><div><dt>招聘批次</dt><dd>{{ company.recruitmentBatch||'未填写' }}</dd></div><div><dt>企业简介</dt><dd>{{ company.description||'未填写' }}</dd></div><div><dt>备注</dt><dd>{{ company.notes||'未填写' }}</dd></div></dl><div class="links"><el-button v-if="company.officialWebsite" :icon="Link" @click="openUrl(company.officialWebsite)">官方网站</el-button><el-button v-if="company.recruitmentWebsite" :icon="Link" type="primary" @click="openUrl(company.recruitmentWebsite)">招聘网站</el-button></div></AppCard>
...
<style scoped lang="scss">.page-stack{display:grid;gap:16px}.detail-grid{display:grid;grid-template-columns:340px minmax(0,1fr);gap:16px}.profile-card,.jobs-card{padding:20px}.profile-card h2,.section-head h2{margin:0 0 16px;font-size:16px}.limit-panel{display:grid;gap:7px;margin-bottom:18px;padding:14px;border:1px solid color-mix(in srgb,var(--teal) 35%,var(--border-color));border-left:4px solid var(--teal);border-radius:7px;background:color-mix(in srgb,var(--teal) 7%,var(--bg-card))}.limit-panel.reached{border-left-color:var(--danger);background:color-mix(in srgb,var(--danger) 6%,var(--bg-card))}.limit-panel span{color:var(--text-secondary);font-size:12px}.limit-panel .status-tag{justify-self:start}.profile-card dl{display:grid;gap:16px;margin:0}.profile-card dl div{display:grid;gap:5px}.profile-card dt{color:var(--text-tertiary);font-size:12px}.profile-card dd{margin:0;line-height:1.7}.links{display:flex;gap:8px;margin-top:20px}.section-head{display:flex;justify-content:space-between}.section-head p{margin:-10px 0 14px;color:var(--text-secondary);font-size:12px}.empty-state{min-height:300px}@media(max-width:1000px){.detail-grid{grid-template-columns:1fr}}</style>
